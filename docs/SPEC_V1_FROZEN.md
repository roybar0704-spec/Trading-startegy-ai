...
- **הבהרה — `median_spread(hour)` חייב להיות Point-in-Time (אושרה במפורש ע"י המשתמש; ר' DECISIONS_LOG D-049):** `median_spread(hour_et)` הנצרך כאן חייב להיגזר **אך ורק** מנתונים שהיו זמינים עד רגע ה-`now` הנוכחי בבקטסט — לעולם לא מ-SpreadReport שנבנה על טווח הכולל נתונים עתידיים ביחס לאותו רגע. **הוכרע (D-055): הווריאנט הנבחר ל-v1 הוא Expanding** (מצטבר מתחילת הריצה, ולא Rolling עם חלון-נגרר) — ומומש בפועל כ-`ExpandingSpreadReport` המחווט ל-Orchestrator. SpreadReport מסוג אחר (למשל אגרגט על פני כל התקופה, או Rolling) הוא זרוע-מחקר נפרדת בלבד ואינו רשאי לשנות את התנהגות v1.
