...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה: בתהליך** — KI-001/KI-002 נסגרו (D-069/D-070), אך היעד המוצהר (3 שנות דאטה אמיתי + דו"ח ספרד אמיתי + כיול RA-10) עדיין לא הושלם — **20 מתוך 39 חודשים קיימים בפועל (אוקטובר 2022–מאי 2024, ~51.3%)**, מאומתים (Batch 1 + Batch 2 + Batch 3 + Batch 4 Closure Reports + Dry Run מלא) | 19 חודשים נותרים (יוני 2024–דצמבר 2025) + דו"ח ספרד אמיתי + RA-10 מכויל |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | **20 מתוך 39 חודשים (~51.3%):** `data/ticks/XAUUSD/2022/{10,11,12}.parquet` + `data/ticks/XAUUSD/2023/{01..12}.parquet` + `data/ticks/XAUUSD/2024/{01..05}.parquet` — כל אחד עם `.sha256` sidecar ורשומת `checkpoint.json` מלאה (`row_count`/`data_version`/`completed_at`). נובמבר 2022, אוקטובר 2022, יוני 2023, ספטמבר 2023 ואפריל 2024 אומתו ב-Hash עצמאי כפול (לא רק דרך ה-checkpoint); שאר החודשים מאומתים דרך ה-checkpoint בלבד (ר' `BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md`/`BATCH3_CLOSURE_REPORT.md`/`BATCH4_CLOSURE_REPORT.md` §6 למגבלות). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת ופעילה בפועל (`scripts/backfill_full_range.py`, רץ **מהמחשב הביתי**, לא מסביבת Sandbox) · **Batch 1 (אוקטובר–דצמבר 2022) — Closed** (`BATCH1_CLOSURE_REPORT.md`) · **Batch 2 (יוני–אוגוסט 2023) — Closed** (`BATCH2_CLOSURE_REPORT.md`) · **Batch 3 (ספטמבר–נובמבר 2023) — Closed** (`BATCH3_CLOSURE_REPORT.md`) · **Batch 4 (דצמבר 2023–מאי 2024, טווח מורחב) — Closed** (`BATCH4_CLOSURE_REPORT.md`) · ינואר–מאי 2023 הושלמו בהרצות קודמות על אותה סביבה · ניתוח Gap/Spike על אוקטובר 2022, יוני-אוגוסט 2023, ספטמבר-נובמבר 2023 ודצמבר 2023-מאי 2024 (KI-022, עדיין לא נסגר) |
| **מה חסר** | **19 מתוך 39 חודשים** (יוני 2024–דצמבר 2025) — כ-48.7% מהדאטה הנדרש עדיין חסר. דו"ח ספרד אמיתי מלא (T0.6) לא הופק על הטווח המלא. RA-10 (Slippage-Stop, כרגע `0.10$` — הערכה ראשונית לא-מכוילת) לא כויל מול מדידה אמיתית. לוח חדשות אמיתי (KI-010) מעולם לא אותר — לא ידוע אפילו אם יש מקור זמין |
| **מצב Backfill** | הסקריפט מוכח ופעיל בפועל (D-069 + Batch 1 + Batch 2 + Batch 3 + Batch 4: 12+8+17+25 שגיאות זמניות טופלו ע"י Retry, כולל 4 Retries ברמת-חודש (ספטמבר 2023 + פברואר/מרץ/אפריל 2024) שהצליחו כולם בניסיון שני, 0 429s, 0 כשלים סופיים בכל ההרצות). כל ההרצות עד כה **מהמחשב הביתי**, לא מה-Sandbox. Dry Run מלא על כל 39 החודשים בוצע ואומת אחרי כל Batch — 20 completed, 19 pending, ללא גישת-רשת |
...
- **המשמעות:** קיימים כרגע **20 מתוך 39 חודשים נדרשים (~51.3%)** — אוקטובר 2022–מאי 2024, מאומתים (Batch 1 + Batch 2 + Batch 3 + Batch 4 Closure + Dry Run מלא). נותרים 19 חודשים (יוני 2024–דצמבר 2025). `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום.
- **למה זה חוסם:** RRR סעיף 4 דורש AT-0.1/0.2/0.6/0.7 מול דאטה אמיתי מלא, לא רק Fixtures/מדגם. גם RA-10 (5.1) וגם KI-022 (5.4) תלויים בכמות-דאטה משמעותית — 20 חודשים כבר מספיקים להתחיל בכיוונים האלה, אך לא לסגור את RRR עצמו.
- **מה נדרש לסגור:** להמשיך את ה-Backfill (מסלול פעיל ומוכח ארבע פעמים — מהמחשב הביתי, Batch 1+2+3+4) על פני 19 החודשים הנותרים, ב-Batches נוספים לפי `BACKFILL_RUN_PLAN.md` (מתעדכן מול Dry Run בכל שלב, לא לפי התכנון המקורי בלבד).

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`). **עדכון:** סריקת Spike נוספת בוצעה על אוקטובר 2022 (Batch 1: 5,106 spikes), יוני-אוגוסט 2023 (Batch 2: 4,113 / 4,345 / 3,403), ספטמבר-נובמבר 2023 (Batch 3: 2,386 / 3,542 / 2,703) ודצמבר 2023-מאי 2024 (Batch 4: 2,673 / 3,244 / 1,993 / 3,047 / 6,951 / 6,571 — שיעור-Spike ו-Row Count גבוהים יותר באפריל-מאי 2024, סביר שמשקף תנודתיות-שוק אמיתית) — MATCH מלא בין Validator לחישוב-מקומי בכל אחד, ללא יוצא מן הכלל. דצמבר 2022/ינואר–מאי 2023 עדיין לא נסרקו (`BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md`/`BATCH3_CLOSURE_REPORT.md`/`BATCH4_CLOSURE_REPORT.md` §6).
...
מסלול מומלץ, בסדר תלות טבעי (מעודכן לאור 20/39):
1. **המשך ה-Backfill (19 חודשים נותרים)** — המסלול כבר **פעיל ומוכח ארבע פעמים בפועל** (Batch 1 + Batch 2 + Batch 3 + Batch 4 סגורים, מהמחשב הביתי) — לא שאלה תיאורטית יותר. Batches נוספים לפי `BACKFILL_RUN_PLAN.md` (מעודכן מול Dry Run בכל שלב).
...
3. **RA-10 + כיול Validator (KI-022)** — כבר יש 20 חודשים אמיתיים (יותר מספיק להתחיל דו"ח-ספרד/כיול ראשוני משמעותי) — לא חייב לחכות לכל 39.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. שלושה חסמים מהותיים (RA-10, KI-010, 3 שנות דאטה) חוסמים את המעבר הזה. **התקדמות אמיתית מדידה:** מ-1/39 חודשים ל-**20/39 (~51.3%)**, דרך תהליך-Backfill מוכח ומתועד ארבע פעמים ברציפות (Batch 1 + Batch 2 + Batch 3 + Batch 4 Closed, Dry Run מלא אחרי כל Batch). KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (Sandbox מול מחשב-בית) **נפתרה בפועל** — המסלול הפעיל (מחשב-בית) עובד ומוכח ארבע פעמים, לא רק תיאורטי. **תובנת-תהליך חדשה מ-Batch 4:** אימות HEAD מקדים דורש `git fetch` מפורש, לא רק `git status` — פער זה זוהה ותוקן במלואו לפני ההרצה, ולא השפיע על תוצאות ה-Backfill (הקבצים שהיו חסרים היו תיעוד בלבד).

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד (בדיוק כפי שציינת) שעבר משלב "מוכן תיאורטית" לשלב "בפועל מזין נתונים אמיתיים, עם Evidence מתועד לכל צעד, כולל Preflight שתפס פערי-תיעוד לפני שהם הצטברו". 19 חודשים נותרים עד סגירת חסם-הדאטה הכמותי; RA-10/KI-010 עדיין דורשים עבודה נפרדת. הצעד הבא הנכון הוא להמשיך את אותו תהליך שכבר הוכח עובד ארבע פעמים — לא לשנות כיוון.
