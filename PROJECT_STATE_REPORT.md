...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה: בתהליך** — KI-001/KI-002 נסגרו (D-069/D-070), אך היעד המוצהר (3 שנות דאטה אמיתי + דו"ח ספרד אמיתי + כיול RA-10) עדיין לא הושלם — **11 מתוך 39 חודשים קיימים בפועל (אוקטובר 2022–אוגוסט 2023, ~28.2%)**, מאומתים (Batch 1 + Batch 2 Closure Reports + Dry Run מלא) | 28 חודשים נותרים (ספטמבר 2023–דצמבר 2025) + דו"ח ספרד אמיתי + RA-10 מכויל |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | **11 מתוך 39 חודשים (~28.2%):** `data/ticks/XAUUSD/2022/{10,11,12}.parquet` + `data/ticks/XAUUSD/2023/{01,02,03,04,05,06,07,08}.parquet` — כל אחד עם `.sha256` sidecar ורשומת `checkpoint.json` מלאה (`row_count`/`data_version`/`completed_at`). נובמבר 2022, אוקטובר 2022 ויוני 2023 אומתו ב-Hash עצמאי כפול (לא רק דרך ה-checkpoint); שאר החודשים מאומתים דרך ה-checkpoint בלבד (ר' `BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md` §6 למגבלות). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת ופעילה בפועל (`scripts/backfill_full_range.py`, רץ **מהמחשב הביתי**, לא מסביבת Sandbox) · **Batch 1 (אוקטובר–דצמבר 2022) — Closed** (`BATCH1_CLOSURE_REPORT.md`) · **Batch 2 (יוני–אוגוסט 2023) — Closed** (`BATCH2_CLOSURE_REPORT.md`) · ינואר–מאי 2023 הושלמו בהרצות קודמות על אותה סביבה · ניתוח Gap/Spike על אוקטובר 2022 ויוני-אוגוסט 2023 (KI-022, עדיין לא נסגר) |
| **מה חסר** | **28 מתוך 39 חודשים** (ספטמבר 2023–דצמבר 2025) — כ-71.8% מהדאטה הנדרש עדיין חסר. דו"ח ספרד אמיתי מלא (T0.6) לא הופק על הטווח המלא. RA-10 (Slippage-Stop, כרגע `0.10$` — הערכה ראשונית לא-מכוילת) לא כויל מול מדידה אמיתית. לוח חדשות אמיתי (KI-010) מעולם לא אותר — לא ידוע אפילו אם יש מקור זמין |
| **מצב Backfill** | הסקריפט מוכח ופעיל בפועל (D-069 + Batch 1 + Batch 2: 12+8 שגיאות זמניות טופלו ע"י Retry, 0 429s, 0 כשלים סופיים בכל ההרצות). כל ההרצות עד כה **מהמחשב הביתי**, לא מה-Sandbox — השאלה אם ניתן להריץ מכאן נשארת פתוחה ולא-רלוונטית כרגע, כי המסלול הפעיל עובד היטב. Dry Run מלא על כל 39 החודשים בוצע ואומת אחרי כל Batch — 11 completed, 28 pending, ללא גישת-רשת |
...
- **המשמעות:** קיימים כרגע **11 מתוך 39 חודשים נדרשים (~28.2%)** — אוקטובר 2022–אוגוסט 2023, מאומתים (Batch 1 + Batch 2 Closure + Dry Run מלא). נותרים 28 חודשים (ספטמבר 2023–דצמבר 2025). `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום.
- **למה זה חוסם:** RRR סעיף 4 דורש AT-0.1/0.2/0.6/0.7 מול דאטה אמיתי מלא, לא רק Fixtures/מדגם. גם RA-10 (5.1) וגם KI-022 (5.4) תלויים בכמות-דאטה משמעותית — 11 חודשים כבר מספיקים להתחיל בכיוונים האלה, אך לא לסגור את RRR עצמו.
- **מה נדרש לסגור:** להמשיך את ה-Backfill (מסלול פעיל ומוכח פעמיים — מהמחשב הביתי, Batch 1+2) על פני 28 החודשים הנותרים, ב-Batches נוספים לפי `BACKFILL_RUN_PLAN.md` (מתעדכן מול Dry Run בכל שלב, לא לפי התכנון המקורי בלבד).

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`). **עדכון:** סריקת Spike נוספת בוצעה על אוקטובר 2022 (Batch 1: 5,106 spikes) ועל יוני-אוגוסט 2023 (Batch 2: 4,113 / 4,345 / 3,403 spikes בהתאמה) — MATCH מלא בין Validator לחישוב-מקומי בכל אחד. דצמבר 2022/ינואר–מאי 2023 עדיין לא נסרקו (`BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md` §6).
...
מסלול מומלץ, בסדר תלות טבעי (מעודכן לאור 11/39):
1. **המשך ה-Backfill (28 חודשים נותרים)** — המסלול כבר **פעיל ומוכח פעמיים בפועל** (Batch 1 + Batch 2 סגורים, מהמחשב הביתי) — לא שאלה תיאורטית יותר. Batches נוספים לפי `BACKFILL_RUN_PLAN.md` (מעודכן מול Dry Run בכל שלב).
...
3. **RA-10 + כיול Validator (KI-022)** — כבר יש 11 חודשים אמיתיים (יותר מספיק להתחיל דו"ח-ספרד/כיול ראשוני משמעותי) — לא חייב לחכות לכל 39.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. שלושה חסמים מהותיים (RA-10, KI-010, 3 שנות דאטה) חוסמים את המעבר הזה. **התקדמות אמיתית מדידה:** מ-1/39 חודשים ל-**11/39 (~28.2%)**, דרך תהליך-Backfill מוכח ומתועד פעמיים ברציפות (Batch 1 + Batch 2 Closed, Dry Run מלא אחרי כל Batch). KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (Sandbox מול מחשב-בית) **נפתרה בפועל** — המסלול הפעיל (מחשב-בית) עובד ומוכח פעמיים, לא רק תיאורטי.

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד (בדיוק כפי שציינת) שעבר משלב "מוכן תיאורטית" לשלב "בפועל מזין נתונים אמיתיים, עם Evidence מתועד לכל צעד, כולל Preflight שתפס פערי-תיעוד לפני שהם הצטברו". 28 חודשים נותרים עד סגירת חסם-הדאטה הכמותי; RA-10/KI-010 עדיין דורשים עבודה נפרדת. הצעד הבא הנכון הוא להמשיך את אותו תהליך שכבר הוכח עובד פעמיים — לא לשנות כיוון.
