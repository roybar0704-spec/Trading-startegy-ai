...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה: בתהליך** — KI-001/KI-002 נסגרו (D-069/D-070), אך היעד המוצהר (3 שנות דאטה אמיתי + דו"ח ספרד אמיתי + כיול RA-10) עדיין לא הושלם — **26 מתוך 39 חודשים קיימים בפועל (אוקטובר 2022–נובמבר 2024, ~66.7%)**, מאומתים (Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5 Closure Reports + Dry Run מלא) | 13 חודשים נותרים (דצמבר 2024–דצמבר 2025) + דו"ח ספרד אמיתי + RA-10 מכויל |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | **26 מתוך 39 חודשים (~66.7%):** `data/ticks/XAUUSD/2022/{10,11,12}.parquet` + `data/ticks/XAUUSD/2023/{01..12}.parquet` + `data/ticks/XAUUSD/2024/{01..11}.parquet` — כל אחד עם `.sha256` sidecar ורשומת `checkpoint.json` מלאה (`row_count`/`data_version`/`completed_at`). נובמבר 2022, אוקטובר 2022, יוני 2023, ספטמבר 2023, אפריל 2024 ואוגוסט 2024 אומתו ב-Hash עצמאי כפול (לא רק דרך ה-checkpoint); שאר החודשים מאומתים דרך ה-checkpoint בלבד (ר' `BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md`/`BATCH3_CLOSURE_REPORT.md`/`BATCH4_CLOSURE_REPORT.md`/`BATCH5_CLOSURE_REPORT.md` §6 למגבלות). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת ופעילה בפועל (`scripts/backfill_full_range.py`, רץ **מהמחשב הביתי**, לא מסביבת Sandbox) · **Batch 1 (אוקטובר–דצמבר 2022) — Closed** (`BATCH1_CLOSURE_REPORT.md`) · **Batch 2 (יוני–אוגוסט 2023) — Closed** (`BATCH2_CLOSURE_REPORT.md`) · **Batch 3 (ספטמבר–נובמבר 2023) — Closed** (`BATCH3_CLOSURE_REPORT.md`) · **Batch 4 (דצמבר 2023–מאי 2024, טווח מורחב) — Closed** (`BATCH4_CLOSURE_REPORT.md`) · **Batch 5 (יוני–נובמבר 2024) — Closed** (`BATCH5_CLOSURE_REPORT.md`) · ינואר–מאי 2023 הושלמו בהרצות קודמות על אותה סביבה · ניתוח Gap/Spike על אוקטובר 2022, יוני-אוגוסט 2023, ספטמבר-נובמבר 2023, דצמבר 2023-מאי 2024 ויוני-נובמבר 2024 (KI-022, עדיין לא נסגר) |
| **מה חסר** | **13 מתוך 39 חודשים** (דצמבר 2024–דצמבר 2025) — כ-33.3% מהדאטה הנדרש עדיין חסר. דו"ח ספרד אמיתי מלא (T0.6) לא הופק על הטווח המלא. RA-10 (Slippage-Stop, כרגע `0.10$` — הערכה ראשונית לא-מכוילת) לא כויל מול מדידה אמיתית. לוח חדשות אמיתי (KI-010) מעולם לא אותר — לא ידוע אפילו אם יש מקור זמין |
| **מצב Backfill** | הסקריפט מוכח ופעיל בפועל (D-069 + Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5: 12+8+17+25+15 שגיאות זמניות טופלו ע"י Retry, כולל 5 Retries ברמת-חודש (ספטמבר 2023, פברואר/מרץ/אפריל 2024, ספטמבר 2024) שהצליחו כולם בניסיון שני, 0 429s, 0 כשלים סופיים בכל ההרצות). כל ההרצות עד כה **מהמחשב הביתי**, לא מה-Sandbox. Dry Run מלא על כל 39 החודשים בוצע ואומת אחרי כל Batch — 26 completed, 13 pending, ללא גישת-רשת |
...
- **המשמעות:** קיימים כרגע **26 מתוך 39 חודשים נדרשים (~66.7%)** — אוקטובר 2022–נובמבר 2024, מאומתים (Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5 Closure + Dry Run מלא). נותרים 13 חודשים (דצמבר 2024–דצמבר 2025). `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום.
- **למה זה חוסם:** RRR סעיף 4 דורש AT-0.1/0.2/0.6/0.7 מול דאטה אמיתי מלא, לא רק Fixtures/מדגם. גם RA-10 (5.1) וגם KI-022 (5.4) תלויים בכמות-דאטה משמעותית — 26 חודשים כבר מספיקים להתחיל בכיוונים האלה, אך לא לסגור את RRR עצמו.
- **מה נדרש לסגור:** להמשיך את ה-Backfill (מסלול פעיל ומוכח חמש פעמים — מהמחשב הביתי, Batch 1+2+3+4+5) על פני 13 החודשים הנותרים, ב-2 Batches נוספים (מדיניות 6-חודשים כברירת-מחדל) לפי `BACKFILL_RUN_PLAN.md` (מתעדכן מול Dry Run בכל שלב).

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`). **עדכון:** סריקת Spike נוספת בוצעה על אוקטובר 2022 (Batch 1: 5,106 spikes), יוני-אוגוסט 2023 (Batch 2: 4,113 / 4,345 / 3,403), ספטמבר-נובמבר 2023 (Batch 3: 2,386 / 3,542 / 2,703), דצמבר 2023-מאי 2024 (Batch 4: 2,673 / 3,244 / 1,993 / 3,047 / 6,951 / 6,571) ויוני-נובמבר 2024 (Batch 5: 6,683 / 9,154 / 10,315 / 8,147 / 9,573 / 13,282 — שיעור-Spike במגמת-עלייה ברורה על פני 2 Batches רצופים, משיא-קודם 0.1723% ל-0.2237% בנובמבר 2024) — MATCH מלא בין Validator לחישוב-מקומי בכל אחד, ללא יוצא מן הכלל. דצמבר 2022/ינואר–מאי 2023 עדיין לא נסרקו (`BATCH1_CLOSURE_REPORT.md`/`BATCH2_CLOSURE_REPORT.md`/`BATCH3_CLOSURE_REPORT.md`/`BATCH4_CLOSURE_REPORT.md`/`BATCH5_CLOSURE_REPORT.md` §6).
...
מסלול מומלץ, בסדר תלות טבעי (מעודכן לאור 26/39):
1. **המשך ה-Backfill (13 חודשים נותרים)** — המסלול כבר **פעיל ומוכח חמש פעמים בפועל** (Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5 סגורים, מהמחשב הביתי) — לא שאלה תיאורטית יותר. 2 Batches נוספים בלבד (מדיניות 6-חודשים כברירת-מחדל) לפי `BACKFILL_RUN_PLAN.md` (מעודכן מול Dry Run בכל שלב).
...
3. **RA-10 + כיול Validator (KI-022)** — כבר יש 26 חודשים אמיתיים (יותר מספיק להתחיל דו"ח-ספרד/כיול ראשוני משמעותי) — לא חייב לחכות לכל 39.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. שלושה חסמים מהותיים (RA-10, KI-010, 3 שנות דאטה) חוסמים את המעבר הזה. **התקדמות אמיתית מדידה:** מ-1/39 חודשים ל-**26/39 (~66.7%)**, דרך תהליך-Backfill מוכח ומתועד חמש פעמים ברציפות (Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5 Closed, Dry Run מלא אחרי כל Batch). KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (Sandbox מול מחשב-בית) **נפתרה בפועל** — המסלול הפעיל (מחשב-בית) עובד ומוכח חמש פעמים, לא רק תיאורטי. **תובנת-תהליך מ-Batch 4, מאושרת שוב ב-Batch 5:** מדיניות Batches של 6 חודשים כברירת-מחדל הוכחה בטוחה פעמיים; אימות HEAD מקדים ממשיך לדרוש `git fetch` מפורש (נתפס נכון ב-Preflight Batch 5 — הפעם המשתמש עצמו זיהה את הפער לפני שדיווח, לא רק תגובה לבקשה).

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד (בדיוק כפי שציינת) שעבר משלב "מוכן תיאורטית" לשלב "בפועל מזין נתונים אמיתיים, עם Evidence מתועד לכל צעד, כולל Preflight שתפס פערי-תיעוד לפני שהם הצטברו". 13 חודשים נותרים עד סגירת חסם-הדאטה הכמותי (2 Batches בלבד לפי המדיניות החדשה); RA-10/KI-010 עדיין דורשים עבודה נפרדת. הצעד הבא הנכון הוא להמשיך את אותו תהליך שכבר הוכח עובד חמש פעמים — לא לשנות כיוון.
