...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה: בתהליך** — KI-001/KI-002 נסגרו (D-069/D-070), אך היעד המוצהר (3 שנות דאטה אמיתי + דו"ח ספרד אמיתי + כיול RA-10) עדיין לא הושלם — **32 מתוך 39 חודשים קיימים בפועל (אוקטובר 2022–מאי 2025, ~82.1%)**, מאומתים (Batch 1 + Batch 2 + Batch 3 + Batch 4 + Batch 5 + Batch 6 Closure Reports + Dry Run מלא) | 7 חודשים נותרים (יוני–דצמבר 2025, Batch יחיד ואחרון) + דו"ח ספרד אמיתי + RA-10 מכויל |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | **32 מתוך 39 חודשים (~82.1%):** `data/ticks/XAUUSD/2022/{10,11,12}.parquet` + `data/ticks/XAUUSD/2023/{01..12}.parquet` + `data/ticks/XAUUSD/2024/{01..12}.parquet` + `data/ticks/XAUUSD/2025/{01..05}.parquet` — כל אחד עם `.sha256` sidecar ורשומת `checkpoint.json` מלאה (`row_count`/`data_version`/`completed_at`). נובמבר 2022, אוקטובר 2022, יוני 2023, ספטמבר 2023, אפריל 2024, אוגוסט 2024 ומרץ 2025 אומתו ב-Hash עצמאי כפול (לא רק דרך ה-checkpoint); שאר החודשים מאומתים דרך ה-checkpoint בלבד (ר' `BATCH1_CLOSURE_REPORT.md`…`BATCH6_CLOSURE_REPORT.md` §6 למגבלות). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת ופעילה בפועל (`scripts/backfill_full_range.py`, רץ **מהמחשב הביתי**, לא מסביבת Sandbox) · **Batch 1-6 — כולם Closed** (`BATCH1_CLOSURE_REPORT.md`…`BATCH6_CLOSURE_REPORT.md`) · ינואר–מאי 2023 הושלמו בהרצות קודמות על אותה סביבה · ניתוח Gap/Spike על כל 32 החודשים הקיימים (KI-022, עדיין לא נסגר) |
| **מה חסר** | **7 מתוך 39 חודשים** (יוני–דצמבר 2025, Batch יחיד ואחרון) — כ-17.9% מהדאטה הנדרש עדיין חסר. דו"ח ספרד אמיתי מלא (T0.6) לא הופק על הטווח המלא. RA-10 (Slippage-Stop, כרגע `0.10$` — הערכה ראשונית לא-מכוילת) לא כויל מול מדידה אמיתית. לוח חדשות אמיתי (KI-010) מעולם לא אותר — לא ידוע אפילו אם יש מקור זמין |
| **מצב Backfill** | הסקריפט מוכח ופעיל בפועל (D-069 + Batch 1-6: 12+8+17+25+15+28 שגיאות זמניות טופלו ע"י Retry, כולל 6 Retries ברמת-חודש שהצליחו כולם — **שיעור-Retry עולה בחדות ב-Batch 6** (4/6 חודשים, 2 מהם בניסיון 3/3 האחרון), 0 429s, 0 כשלים סופיים בכל ההרצות). כל ההרצות עד כה **מהמחשב הביתי**, לא מה-Sandbox. Dry Run מלא על כל 39 החודשים בוצע ואומת אחרי כל Batch — 32 completed, 7 pending, ללא גישת-רשת |
...
- **המשמעות:** קיימים כרגע **32 מתוך 39 חודשים נדרשים (~82.1%)** — אוקטובר 2022–מאי 2025, מאומתים (Batch 1-6 Closure + Dry Run מלא). נותרים 7 חודשים (יוני–דצמבר 2025) — Batch יחיד ואחרון. `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום.
- **למה זה חוסם:** RRR סעיף 4 דורש AT-0.1/0.2/0.6/0.7 מול דאטה אמיתי מלא, לא רק Fixtures/מדגם. גם RA-10 (5.1) וגם KI-022 (5.4) תלויים בכמות-דאטה משמעותית — 32 חודשים כבר מספיקים להתחיל בכיוונים האלה, אך לא לסגור את RRR עצמו.
- **מה נדרש לסגור:** להמשיך את ה-Backfill (מסלול פעיל ומוכח שש פעמים — מהמחשב הביתי, Batch 1-6) על פני 7 החודשים האחרונים, ב-Batch 7 (יחיד, סופי) לפי `BACKFILL_RUN_PLAN.md`.

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`). **עדכון:** סריקת Spike נוספת בוצעה על כל 32 החודשים הקיימים (Batch 1-6, ר' Closure Reports לפירוט מלא) — MATCH מלא בין Validator לחישוב-מקומי בכל אחד, ללא יוצא מן הכלל. **תצפית מצטברת ומשמעותית מ-Batch 6:** קפיצה חדה בגודל-Move (חציוני **וגם** מקסימלי) באפריל-מאי 2025 — חציוני 0.55-0.62 (מול ~0.28-0.33 בחודשים שקדמו), מקסימלי 30.9/51.7 (מול ~5-14 קודם לכן) — הנקודה הבולטת ביותר עד כה לכיול. דצמבר 2022/ינואר–מאי 2023 עדיין לא נסרקו (`BATCH1_CLOSURE_REPORT.md`…`BATCH6_CLOSURE_REPORT.md` §6).
...
מסלול מומלץ, בסדר תלות טבעי (מעודכן לאור 32/39):
1. **סיום ה-Backfill (7 חודשים נותרים, Batch יחיד ואחרון)** — המסלול כבר **פעיל ומוכח שש פעמים בפועל** (Batch 1-6 סגורים, מהמחשב הביתי) — לא שאלה תיאורטית יותר. Batch 7 (`2025-06-01…2025-12-31`) הוא ה-Gate הסופי (39/39) לפי `BACKFILL_RUN_PLAN.md`.
...
3. **RA-10 + כיול Validator (KI-022)** — כבר יש 32 חודשים אמיתיים (יותר מספיק להתחיל דו"ח-ספרד/כיול ראשוני משמעותי) — לא חייב לחכות לכל 39.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. שלושה חסמים מהותיים (RA-10, KI-010, 3 שנות דאטה) חוסמים את המעבר הזה. **התקדמות אמיתית מדידה:** מ-1/39 חודשים ל-**32/39 (~82.1%)**, דרך תהליך-Backfill מוכח ומתועד שש פעמים ברציפות (Batch 1-6 Closed, Dry Run מלא אחרי כל Batch). KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (Sandbox מול מחשב-בית) **נפתרה בפועל** — המסלול הפעיל (מחשב-בית) עובד ומוכח שש פעמים, לא רק תיאורטי. **תובנת-תהליך חדשה מ-Batch 6:** שיעור-Retry ברמת-חודש עלה בחדות (4/6, 2 בניסיון האחרון) — לא הביא לכשל בפועל, אך מגמה למעקב ב-Batch 7 האחרון. גם קפיצה חדה בגודל-Move (לא רק Row-Count) נצפתה לראשונה — נקודת-נתון בולטת ל-KI-022.

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד (בדיוק כפי שציינת) שעבר משלב "מוכן תיאורטית" לשלב "בפועל מזין נתונים אמיתיים, עם Evidence מתועד לכל צעד, כולל Preflight שתפס פערי-תיעוד לפני שהם הצטברו". **7 חודשים בלבד נותרים, ב-Batch יחיד ואחרון (Batch 7)** עד סגירת חסם-הדאטה הכמותי; RA-10/KI-010 עדיין דורשים עבודה נפרדת. הצעד הבא הנכון הוא להשלים את אותו תהליך שכבר הוכח עובד שש פעמים — לא לשנות כיוון.
