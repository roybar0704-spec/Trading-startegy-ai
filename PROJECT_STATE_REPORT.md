...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה: עדיין לא הושלם** — KI-001/KI-002 נסגרו (D-069/D-070), אך היעד המוצהר (3 שנות דאטה אמיתי + דו"ח ספרד אמיתי + כיול RA-10) עדיין לא קיים — רק חודש אחד (נובמבר 2022) ירד ואומת בפועל | 3 שנות דאטה + דו"ח ספרד אמיתי + RA-10 מכויל |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | חודש אחד בלבד: `data/ticks/XAUUSD/2022/11.parquet` (3,641,776 Ticks אמיתיים, מאומת מול D-069/D-070: row_count/bid/ask/schema תואמים בדיוק). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים — אף ריצה לא צרפה Journal אמיתי עד כה) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת (`scripts/backfill_full_range.py`, מאומתת על חודש שלם **מהמחשב הביתי**, לא מסביבת Sandbox) · ניתוח Gap/Spike ראשוני (KI-022, לא נסגר) |
| **מה חסר** | **~38 מתוך ~39 חודשים** (טווח 3 שנים + Warm-Up 90 יום) — כ-2.5% מהדאטה הנדרש קיים בפועל. דו"ח ספרד אמיתי מלא (T0.6) לא הופק על הטווח המלא. RA-10 (Slippage-Stop, כרגע `0.10$` — הערכה ראשונית לא-מכוילת) לא כויל מול מדידה אמיתית. לוח חדשות אמיתי (KI-010) מעולם לא אותר — לא ידוע אפילו אם יש מקור זמין |
| **מצב Backfill** | הסקריפט קיים ומוכח (D-069: 720/0/0 = קריאות/429/שגיאות על חודש שלם), אך **לא הורץ שוב** מאז. שאלה פתוחה שעדיין לא נענתה במלואה: האם ניתן להריץ Backfill אמיתי מתוך סביבת Claude Code Sandbox המרוחקת (זו שבה מופק דוח זה) — ה-403 המקורי של KI-001 תואר במפורש כחסימה **ברמת ה-egress proxy של הסביבה עצמה**, וההוכחה היחידה ל-E2E תקין בוצעה מהמחשב הביתי, לא מכאן |
...
- **המשמעות:** קיים כרגע 1 מתוך ~39 חודשים נדרשים (~2.5%). `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום.
- **למה זה חוסם:** RRR סעיף 4 דורש AT-0.1/0.2/0.6/0.7 מול דאטה אמיתי מלא, לא רק Fixtures/מדגם. גם RA-10 (5.1) וגם KI-022 (5.4) תלויים בכמות-דאטה משמעותית, לא בחודש בודד.
- **מה נדרש לסגור:** להשלים את ה-Backfill (הסקריפט קיים ומוכח) על פני הטווח המלא — **בתנאי שנפתרת שאלת-הרשת** (ר' §3 למעלה: לא ידוע אם ניתן מתוך Sandbox זה, או שנדרש שוב "מחשב הבית").

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`).
...
מסלול מומלץ, בסדר תלות טבעי:
1. **KI-010 (לוח חדשות)** — ניתן להתחיל בו **עצמאית מ-KI-001/רשת-Dukascopy** (מקור נתונים שונה לגמרי), וכנראה לא תלוי בשאלת ה-Sandbox-vs-מחשב-בית. **מועמד עם ה-ROI הכי גבוה להתחיל בו ראשון** — עצמאי, לא-חסום, וסוגר את הסעיף ה-high-severity היחיד שפתוח.
2. **השלמת ה-Backfill (3 שנות דאטה)** — דורש קודם הכרעה על **איפה** בפועל זה ירוץ (Sandbox הזה מול מחשב הבית) — שאלה שעדיין לא נענתה במפורש למרות שעלתה כמה פעמים בשיחות קודמות.
3. **RA-10 + כיול Validator (KI-022)** — טבעי לבצע **אחרי** שיש יותר מחודש דאטה (אפילו 2-3 חודשים נוספים יספיקו כדי לקבל דו"ח-ספרד/כיול משמעותיים יותר מחודש בודד) — לא חייב לחכות לשלושת השנים המלאות כדי להתחיל.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. שלושה חסמים מהותיים (RA-10, KI-010, 3 שנות דאטה) חוסמים את המעבר הזה, ואף אחד מהם לא זז מאז שזוהו. ~2.5% בלבד מהדאטה הנדרש קיים בפועל. KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (איפה מריצים Backfill אמיתי — Sandbox או מחשב-בית) עדיין לא הוכרעה במפורש.

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד (בדיוק כפי שציינת) שממשיך לבנות ולוודא תשתית, אבל טרם עבר את קו-המים לדאטה-אמיתי-בקנה-מידה שהמחקר עצמו דורש. הצעד הבא הנכון הוא לסגור את פער-הדאטה, לא להוסיף עוד הקשחת-קוד.
