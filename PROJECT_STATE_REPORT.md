...
|---|---|---|---|
| **Phase 0** — Data Pipeline | T0.1–T0.6: שלד ריפו, `DukascopyDownloader`, Tick→Parquet, Validator, BarBuilder, דו"ח ספרד | **תת-שער קוד: סגור** (AT-0.1–AT-0.7 ירוקות מול Fixtures). **תת-שער דאטה — 39/39 חודשים הושלם (100%)!** KI-001/KI-002 נסגרו (D-069/D-070), וכעת גם **דרישת-הכמות המלאה של 3 שנות דאטה אמיתי הושלמה** (אוקטובר 2022–דצמבר 2025), מאומתת (Batch 1-7 Closure Reports, `BATCH7_CLOSURE_REPORT.md` — Gate סופי). **עדיין לא הושלם:** דו"ח ספרד אמיתי מלא + כיול RA-10 (עכשיו אפשריים במלואם לראשונה) | דו"ח ספרד אמיתי + RA-10 מכויל + KI-010 (לוח חדשות) — **לא** נסגרים אוטומטית ע"י השלמת ה-Backfill |
...
| פריט | מצב בפועל |
|---|---|
| **מה קיים** | **39 מתוך 39 חודשים (100%) — הטווח המלא הושלם:** `data/ticks/XAUUSD/2022/{10,11,12}.parquet` + `2023/{01..12}` + `2024/{01..12}` + `2025/{01..12}` — כל אחד עם `.sha256` sidecar ורשומת `checkpoint.json` מלאה. נובמבר 2022, אוקטובר 2022, יוני 2023, ספטמבר 2023, אפריל 2024, אוגוסט 2024, מרץ 2025 ואוקטובר 2025 אומתו ב-Hash עצמאי כפול; שאר החודשים מאומתים דרך ה-checkpoint בלבד (ר' `BATCH1_CLOSURE_REPORT.md`…`BATCH7_CLOSURE_REPORT.md` §6 למגבלות). `data/registry/` (מבנה בלבד, `runs.jsonl` עדיין לא קיים) |
| **מה הושלם** | Root cause + תיקון חסימת-רשת (KI-001, D-069) · אימות `point_value` (KI-002, D-070) · תשתית Backfill מוכחת ופעילה בפועל (`scripts/backfill_full_range.py`, רץ **מהמחשב הביתי**, לא מסביבת Sandbox) · **Batch 1-7 — כולם Closed, כולל Batch 7 הסופי** (`BATCH1_CLOSURE_REPORT.md`…`BATCH7_CLOSURE_REPORT.md`) · **✅ 39/39 חודשים — יעד-הכמות של ה-Backfill הושלם במלואו** · ינואר–מאי 2023 הושלמו בהרצות קודמות על אותה סביבה · ניתוח Gap/Spike על כל 39 החודשים (KI-022, עדיין לא נסגר) |
| **מה חסר** | **0 מתוך 39 חודשים דאטה** — הכמות הושלמה. **מה עדיין חסר בפועל לסגירת RRR:** דו"ח ספרד אמיתי מלא (T0.6) על 39 החודשים — טרם הופק. RA-10 (Slippage-Stop, כרגע `0.10$`) — טרם כויל. לוח חדשות אמיתי (KI-010) — מעולם לא אותר. Hash/Spike-Scan עצמאי שני על ינואר-מאי 2023 — טרם בוצע. הרצת AT-0.1/0.2/0.6/0.7 מול הדאטה המלא — טרם בוצעה |
| **מצב Backfill** | **הושלם במלואו.** הסקריפט מוכח ופעיל בפועל שבע פעמים (D-069 + Batch 1-7: 12+8+17+25+15+28+46 שגיאות זמניות טופלו ע"י Retry, כולל Retries ברמת-חודש שהצליחו כולם — שיעור-Retry עלה בחדות ב-Batch 6-7 (4/6, 4/7, כל אחד עם מקרה בניסיון 3/3 האחרון), 0 429s, 0 כשלים סופיים בכל 7 ההרצות. **סטיית-תהליך יחידה:** Dry Run לא בוצע לפני הרצת Batch 7 עצמה (ר' `BATCH7_CLOSURE_REPORT.md` §0) — לא השפיע על שלמות-הנתונים). כל ההרצות **מהמחשב הביתי**, לא מה-Sandbox. Dry Run/checkpoint אחרון: **39 completed, 0 pending** |
...
### 5.3 דרישת 3 שנות דאטה — ✅ סגור (הכמות בלבד; RRR עצמו עדיין NO-GO)
- **המשמעות:** **39 מתוך 39 חודשים (100%) הושלמו** — אוקטובר 2022–דצמבר 2025, מאומתים במלואם (Batch 1-7 Closure, כולל `BATCH7_CLOSURE_REPORT.md` §2 — 39/39 מאושר תכנותית, Missing=∅). `config/run_default.yaml` מגדיר תקופה 2023-01-01…2025-12-31 + Warm-Up 90 יום — כעת מכוסה במלואה.
- **למה זה עדיין לא סוגר את RRR:** RRR סעיף 4 דורש **גם** AT-0.1/0.2/0.6/0.7 להתבצע בפועל מול הדאטה האמיתי המלא — **טרם בוצע**, רק הדאטה עצמו קיים. RA-10 (5.1) וגם KI-022 (5.4) תלויים בדאטה, שכעת קיים במלואו, אבל דורשים עוד עבודה נפרדת (דו"ח-ספרד, ניסוי-כיול) שלא בוצעה עדיין.
- **מה נדרש כעת (לא Backfill — עיבוד על הדאטה הקיים):** דו"ח-ספרד אמיתי מלא (`build_spread_report`) על 39 החודשים → הרצת AT-0.1/0.2/0.6/0.7 → Hash/Spike-Scan עצמאי שני על ינואר-מאי 2023 (עדיין לא בוצע). ר' `BATCH7_CLOSURE_REPORT.md` §7.5 להמלצה מסודרת.

### 5.4 KI-022 — כיול Validator לא נבדק מול דאטה רב-session
- **המשמעות:** חודש בודד (נובמבר 2022) חשף 18 gaps ו-2,998 spikes מדוגללים — רובם מוסברים כהתנהגות-שוק טבעית, אך גודל-move חציוני של ה-spikes (0.2305) קטן מהספרד הטיפוסי, מרמז על רגישות-יתר אפשרית של ה-baseline (`spike_z_threshold=8.0`). **עדכון סופי (Batch 1-7, כל 39 החודשים):** MATCH מלא בין Validator לחישוב-מקומי בכל אחד מ-39 החודשים, ללא יוצא מן הכלל אחד. **מגמה מצטברת לאורך כל הפרויקט:** שיעור-Spike עלה מ-~0.10-0.15% (2022-2023) ל-~0.15-0.22% (2024-2025); גודל-Move חציוני עלה מ-~0.22-0.33 ל-~0.35-0.74, עם קיצוניים חדים ביותר באפריל-מאי-אוקטובר 2025 (מקסימלי עד 51.7). **התנאי המקדים לניסוי-כיול (D-071) — נפח-דאטה מספק — מתקיים כעת במלואו לראשונה.** דצמבר 2022/ינואר–מאי 2023 עדיין ללא Hash/Spike-Scan עצמאי שני (`BATCH1_CLOSURE_REPORT.md`…`BATCH7_CLOSURE_REPORT.md` §6).
- **למה זה חוסם:** אינו ברשימת 9 הסעיפים הפורמליים של RRR, אך נוגע ישירות לסעיף 4 שם ("דאטה נקי"). Validator שלא מבחין בין Corruption/Market-Behavior/Calibration עלול להסתיר בעיות אמיתיות או להציף false positives.
- **מה נדרש לסגור:** Work Order ייעודי לניסוי-כיול מבוקר (D-071) — **טרם נפתח, בהמתנה לאישורך המפורש** (הוחלט במפורש שלא לפתוח ללא הנחייה). כעת אפשרי במלואו לראשונה מבחינת נפח-דאטה.
...
מסלול מומלץ, בסדר תלות טבעי (**מעודכן לאור 39/39 — ה-Backfill הושלם**):
1. **דו"ח-ספרד אמיתי מלא** (`build_spread_report`) על כל 39 החודשים → קלט ישיר לכיול RA-10. **אפשרי במלואו לראשונה כעת.**
2. **KI-010 (לוח חדשות)** — עדיין עצמאי לגמרי מה-Backfill (מקור-נתונים שונה), ועדיין הסעיף ה-high-severity היחיד הפתוח — להתקדם בו **במקביל**.
3. **RA-10 + פתיחת Work Order ל-KI-022** — הדאטה קיים במלואו כעת; ניסוי-הכיול המבוקר (D-071) יכול להיפתח בכפוף לאישורך.
4. **Follow-up:** Hash/Spike-Scan עצמאי שני על ינואר-מאי 2023, לפני הסתמכות מלאה עליהם לצורך RRR.
...
**למה לא GREEN:** **הפרויקט עדיין לא ביצע אף ריצת-מחקר אמיתית אחת** — לא Phase 4 התחיל, לא T3.4 בוצע. **חסם-הדאטה הכמותי נסגר במלואו (39/39, 100%)** — אך RA-10 ו-KI-010 **לא נסגרים אוטומטית** על ידי כך, ונשארים NO-GO. **התקדמות אמיתית מדידה:** מ-1/39 חודשים ל-**39/39 (100%)**, דרך תהליך-Backfill מוכח ומתועד שבע פעמים ברציפות (Batch 1-7 Closed, כולל Gate סופי). KI-010 (high severity) עדיין פתוח — ה-Functional Gate הפורמלי לא נקי לחלוטין. שאלת-התשתית (Sandbox מול מחשב-בית) **נפתרה בפועל לחלוטין** — המסלול הפעיל (מחשב-בית) הוכח שבע פעמים ברציפות, כולל ה-Batch המורכב והגדול ביותר. **תובנות-תהליך מ-Batch 7 (האחרון):** (א) סטיית-תהליך מוצהרת — Dry Run לא בוצע לפני ההרצה, לא השפיע על שלמות-הנתונים; (ב) פער-זמן חריג בלתי-מוסבר של ~5 שעות באמצע הריצה (יולי 2025) — תועד כאנומליית-סביבה, לא נתונים; (ג) Row Count הגיע לשיא של 9.3M (אוקטובר 2025) — אומת במלואו כתקין (MATCH פרופורציונלי).

**נקודת האיזון:** אין כאן "פרויקט תקוע" — יש כאן פרויקט עם משמעת-תהליך גבוהה מאוד שהשלים הישג משמעותי: **3 שנות דאטה אמיתי, מאומת חודש-אחר-חודש, ללא יוצא מן הכלל, לאורך 7 Batches נפרדים.** זו לא סגירת-RRR, אך היא מסירה את החסם הכמותי לחלוטין ופותחת את הדלת ל-RA-10/KI-022 (שדורשים דאטה) לראשונה במלואם. KI-010 נשאר עצמאי ופתוח. הצעד הבא הנכון הוא לעבור מ**"אוסף דאטה"** ל**"עיבוד הדאטה שנאסף"** — דו"ח-ספרד, כיול, ולא הרחבת-דאטה נוספת.
